...
// Code Variables
int timeOut                = 1;      // Deffined in frequencyCalculations (IRpulse + 50)
...
void receiveIR() { // Void checks for an incoming signal and decodes it if it sees one.
    ...
    if(digitalRead(IRreceivePin) == LOW){    // If the receive pin is low a signal is being received.
        ...
        for(int i = 1; i <= 17; i++) {  // Looks at each one of the received pulses
            ...
            if(received[i] > (IRpulse + IRpulse - 200) &&  received[i] < (IRpulse + IRpulse + 400)) {receivedTemp[i] = 1;}  // Works out from the pulse length if it was a data 1 or 0 that was received
...
void frequencyCalculations() { // Works out all the timings needed to give the correct carrier frequency for the IR signal
    ...
    timeOut = IRpulse + 10000; // Adding 50 to the expected pulse time gives a little margin for error on the pulse read time out value
}
